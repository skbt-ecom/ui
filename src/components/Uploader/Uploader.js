...
function Uploader(props) {
  ...
  function Upload(props) {
    ...
    const active = loadState.isLoad || loadState.isSuccess || isDragActive ? classes.active : ""
    ...
    return (
      <>
        ...
        {loadState.isSuccess && (
          <Button classes={{ root: classes.removeBtn }} onClick={removeImage}>
            <Close className={classes.removeIcon} />
          </Button>
        )}
        {loadState.isError && <Typography className={classes.errMsg}>{errMsg}</Typography>}
        <input
          {...getInputProps({
            accept: "image/*",
            multiple: false,
